...
Spawn with **`subagent_type: tandy-<profile>[-capability]`**. That
parameter name is load-bearing: the Agent tool ignores unrecognised parameters
silently, so a typo like `subject_type` does not error — it drops you to a
